package com.rifeng.p2p.tablayout;
...
import com.rifeng.p2p.util.PhoneUtil;

import java.util.ArrayList;
import java.util.List;

/**
 * Created by caizhiming on 2016/3/8.
 */
public class XCTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private List<TextView> mDotList = new ArrayList<>();
    private List<Integer> mDotDataList = new ArrayList<>();
    public void hideAllDot(){
        for (int i = 0; i < mDotList.size(); i++) {
           mDotList.get(i).setVisibility(GONE);
        }
    }
    public void subDotData(int pos){
        int old = mDotDataList.get(pos);
        int newNum = old -1 ;
        if(newNum <=0) newNum = 0;
        mDotDataList.set(pos,newNum);
        if(newNum > 0){
            mDotList.get(pos).setVisibility(VISIBLE);
            mDotList.get(pos).setText(String.valueOf(newNum));
            if(newNum > 99){
                mDotList.get(pos).setText("99+");
                int padding = PhoneUtil.dip2px(mContext,4);
                mDotList.get(pos).setPadding(padding,0,padding,0);
                mDotList.get(pos).requestLayout();
            }else{
                mDotList.get(pos).setText(String.valueOf(newNum));
                int padding = PhoneUtil.dip2px(mContext,0);
                mDotList.get(pos).setPadding(padding,0,padding,0);
                mDotList.get(pos).requestLayout();

            }
        }else{
            mDotList.get(pos).setVisibility(GONE);
        }
    }
    public void setDotDatas(List<Integer> data){
        if(data.size() != (mDotList.size()+1))
            return;
        for (int i = 0; i < mDotList.size(); i++) {
            final TextView dot = mDotList.get(i);
            int num = data.get(i+1);
            mDotDataList.add(num);
            if(num > 0){
                ...
                if(num > 99){
                    ...
                    dot.setText(String.valueOf(num));
                    int padding = PhoneUtil.dip2px(mContext,0);
                    dot.setPadding(padding,0,padding,0);
                    dot.requestLayout();
                }
...
//    }
    ...
    public void notifyDataSetChanged() {
        ...
        for (int i = 0; i < mTabCount; i++) {
            ...
            TextView dot = (TextView) tabView.findViewById(R.id.tv_reddot);
            mDotList.add(dot);
//            if(mShowRedDot){
//                dot.setVisibility(VISIBLE);
//            }else{
